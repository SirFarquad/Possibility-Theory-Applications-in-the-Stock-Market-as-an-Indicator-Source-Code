...
def calculate_mac(data):
    ...
    # Standardizing MAC such that the long moving average corresponds to 0.5
...
def run_possibility_simulation(data):
    ...
    # Calculate RSI and MAC
    data = calculate_rsi(data)
    data = calculate_mac(data)

    # Calculate Possibility Theory Indicator (Min of RSI and MAC)
    data['Possibility'] = data[['RSI', 'MAC']].max(axis=1)
    ...
    plt.title('SPY Closing Price vs. Trading Capital (Possibility Theory: RSI, MAC)')
    plt.xlabel('Date')
