...
def run_rsi_simulation(data):
    ...
    for i, row in data.iterrows():
        if pd.notna(row['RSI']):
            if row['RSI'] < 40 and position == 0:  # Buy Signal
                position = capital / row['SPY_Close']
                capital = 0
            elif row['RSI'] > 60 and position > 0:  # Sell Signal
    ...
    plt.title('SPY Closing Price vs. Trading Capital (RSI Strategy)')
    plt.xlabel('Date')
    ...
    plt.figtext(0.2, 0.8, f'RSI Return: {capital_df["Return"].iloc[-1]:.2f}%', fontsize=12, color='orange')
    plt.figtext(0.2, 0.75, f'RSI Drawdown: {capital_df["Drawdown"].max() * 100:.2f}%', fontsize=12, color='orange')

    plt.show()

if __name__ == "__main__":
    data = fetch_data()
    run_rsi_simulation(data)
