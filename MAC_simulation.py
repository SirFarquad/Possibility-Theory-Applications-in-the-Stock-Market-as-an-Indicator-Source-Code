...
def calculate_mac(data):
    # Moving Average Crossover Calculation
    data['MA_Short'] = data['SPY_Close'].rolling(window=50).mean()  # Short-term Moving Average
    data['MA_Long'] = data['SPY_Close'].rolling(window=200).mean()  # Long-term Moving Average
    return data

def run_mac_simulation(data):
    ...
    for i, row in data.iterrows():
        if pd.notna(row['MA_Short']) and pd.notna(row['MA_Long']):
            if row['MA_Short'] > row['MA_Long'] and position == 0:  # Buy Signal
                position = capital / row['SPY_Close']
                capital = 0
            elif row['MA_Short'] < row['MA_Long'] and position > 0:  # Sell Signal
    ...
    plt.title('SPY Closing Price vs. Trading Capital (MAC Strategy)')
    plt.xlabel('Date')
    ...
    plt.figtext(0.2, 0.8, f'MAC Return: {capital_df["Return"].iloc[-1]:.2f}%', fontsize=12, color='orange')
    plt.figtext(0.2, 0.75, f'MAC Drawdown: {capital_df["Drawdown"].max() * 100:.2f}%', fontsize=12, color='orange')

    plt.show()

if __name__ == "__main__":
    data = fetch_data()
    run_mac_simulation(data)
